...
Задачи 1–7 написаны и покрыты тестами (149 зелёных), фича выключена
(`CLIENT_MESSAGING_ENABLED=0`). Не сделано и требует владельца:

- **Живая проверка событий.** `scripts/check_order_created_events.py` показывает,
  приходят ли переходы в «Заказ оформлен» и заполнена ли дата работы. Запускать
  на сервере: локально токена amoCRM нет.
- **Права на запись в CRM.** Клиент бота до сих пор только читал; проверить,
  разрешает ли токен двигать сделку, можно только на живом API. Отказ в правах
  не молчит: владелец получает сообщение «клиент подтвердил, сделку подвинуть
  не удалось».
- **Деплой и включение** по порядку из раздела «Порядок включения».

Два отличия от плана, принятые по ходу:

1. Статуса `asked` нет: строка ожидания живёт как `planned`, а `asked_at`
   хранит **плановое** время вопроса. Владелец решил, что причина молчания роли
   не играет, — значит отсчёт трёх часов не должен зависеть от того, ушло письмо
   или нет.
2. Непонятый ответ отдаётся владельцу только если его не забрала старая ветка
   (оценка работы, STOP, отклик на промо). Иначе «5» от клиента, ждущего
   подтверждения, потерялось бы как оценка прошлого заказа.
