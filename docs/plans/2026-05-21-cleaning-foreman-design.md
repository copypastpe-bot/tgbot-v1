...
1. **Бонусы за уборку** — формула и проценты точно как в химчистке (переиспользуем существующую логику начисления и списания).
2. **`/cleaning_cancel_order`** — soft-delete всех привязанных к заказу строк `cleaning_cashbook` (и прихода, и расходов) + откат бонусов клиента + оповещение в чат.

## Открытые вопросы (отложено, не блокирует MVP)

1. **Transfer между общей кассой и кассой клининга** — пока не нужен. Если возникнет, добавим `kind='transfer'` парой записей в обеих кассах.
