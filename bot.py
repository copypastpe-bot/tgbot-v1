...
async def help_cmd(msg: Message):
    # если есть права добавлять мастеров, значит пользователь админ/суперадмин
    if await has_permission(msg.from_user.id, "add_master"):
        await msg.answer(
            "Доступно:\n"
            "• /find +7XXXXXXXXXX — карточка клиента\n"
            "• Кнопка «🧾 Я ВЫПОЛНИЛ ЗАКАЗ» — пошаговый ввод заказа\n"
            "• /payroll YYYY-MM — отчёт по зарплате за месяц (админ)\n"
    ...
    # если админских прав нет, но есть право на собственные отчёты, значит это мастер
    elif await has_permission(msg.from_user.id, "view_own_salary"):
        await msg.answer(
            "Доступно:\n"
            "• /find +7XXXXXXXXXX — карточка клиента\n"
        ...
        # базовый набор команд на случай, если пользователь не имеет ролей
        await msg.answer(
            "Доступно:\n"
            "• /find +7XXXXXXXXXX — карточка клиента",
            reply_markup=main_kb
