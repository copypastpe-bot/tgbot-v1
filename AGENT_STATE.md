...
The project is described as fully working, with `bot.py` as the real runtime core. The repository has a large `bot.py` runtime, Postgres-oriented migrations under `app/migrations/`, notification modules under `notifications/`, Wahelp integration helpers under `crm/`, and operational scripts under `scripts/`. A shared `service_heartbeats` table plus a periodic checker in `bot.py` now monitor the companion client Telegram bot and notify admins when its heartbeat or Telegram probe goes stale. The backup and restart operational scripts are now expected to be tracked in git so deploys do not silently drop them.

## Active Focus

Preserve current admin, master, notification, payroll, and cashflow flows while reducing operational drift around deploy/runtime scripts and preparing future Google Calendar work.
...
- Operational scripts must stay committed and deployed from tracked state; missing untracked scripts can break cron jobs without touching runtime code.
